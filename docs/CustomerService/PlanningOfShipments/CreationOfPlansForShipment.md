...
Затем нажимаем кнопку **"Обновить"**, на форме будут выведены созданные ранее в системе "Заказы клиентов" с выбранной датой отгрузки.
...
- *по получателям* - в этом случае будет сформирован общий план отгрузки для каждого получателя заказа  
...
- По получателям (формируется общий план отгрузки по всем заказам с этого склада для одного получателя на одну дату отгрузки)
...
При изменении заказа регламентное задание при повтором запуске автоматически скорректирует план отгрузки.
