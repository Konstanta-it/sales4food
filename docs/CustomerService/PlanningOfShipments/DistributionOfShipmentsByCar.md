...
Если планируется отгрузка, то в качестве распоряжения на доставку выступает документ *"План отгрузки"*. Если предусмотрен самовывоз, то **"Задание на доставку"** создается вручную или автоматически при формировании плана отгрузки, если установлен признак **Автоматически формировать задания на доставку по заказам с самовывозом** в настройках исполнения заказов.

Если планируется поставка, то в качестве распоряжения на доставку выступает документ *"Заявка на возврат товаров от клиента"*.
...
- Тип доставки  
- Склад (так же может использоваться группа складов, тогда к распределению попадут *"Планы отгрузок"* со всех складов из группы)
- Зона
- Дата отгрузки
...
1. *Выбор транспортного средства*
    ...
    По кнопку **Сформировать задание** будет создано задание на доставку с заполненными только перевозчиком, складом, водителем, транспортным средством, прицепом.
...
2. *Распределение планов отгрузки по машинам*
    ...
    В левой таблице АРМа выводятся планы отгрузки и основная информация по ним: дата и время доставки, адрес, получатель, склад, вес, кол-во мест.  
    ...
    Если в плане отгрузки указано время доставки, то оно более приоритетно, чем время, указанное в шаблоне.
...
Для транспортной документации производится расчет количества мест и веса (вес брутто = вес товара + вес коробки). Занимаемое количество мест рассчитывается исходя из информации в карточке номенклатуры об объеме, занимаемом товаром. Для типа транспортного средства заполняются грузоподъемность и вместимость. 
...
Если планируется доставка с другого склада (склад отгрузки в заказе отличается от склада отгрузки в плане отгрузки), то можно добавить необохимое распоряжение в задание на доставку.  

В документе Задание на доставку на вкладке Маршрут по кнопке Добавить открывается форма выбора распоряжений.  
По кнопке Нераспределенные показываются только планы отгрузки, которые не были распределены в задания на доставку. По кнопке Все распоряжения выводятся все планы отгрузки на дату отгрузки со склада.  
При двойном нажатии на план отгрузки или по стрелочке адрес план отгрузки выберется для переноса и в левой части станут доступны для редактирования адрес и зона доставки плана отгрузки.  
По кнопке Перенести в задание выбранные планы отгрузки перенесутся в текущее задание на доставку.
...
[20]: DistributionOfShipmentsByCar.assets/20.png
