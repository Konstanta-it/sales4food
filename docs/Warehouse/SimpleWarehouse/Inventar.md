...
Дозаполнить документ по учету можно двумя способами: по факту и по плану и факту. В первом случае дозаполняется вся продукция, которой не было в пересчете, и отражается со значением 0. Во втором случае заполняет заполняет всю продукцию, которой не было в пересчете со значениями, равными плановым (по учету на складах).
